...
#define	BULLET_UP_POS		(15.0f)			// 弾の高さを上げる
#define BULLET_DISAPPEAR_TIME	(300.0f)	// 弾が消えるまでの時間
...
void UpdatePlayerBullet(void)
{
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		if (g_Bullet[i].use)
		{
			...
			g_Bullet[i].time++;
			// 
			if (g_Bullet[i].time == BULLET_DISAPPEAR_TIME)
			{
...
int SetPlayerBullet(XMFLOAT3 pos, XMFLOAT3 rot)
{
	...
	for (int nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		if (!g_Bullet[nCntBullet].use)
		{
			...
			g_Bullet[nCntBullet].pos.y += BULLET_UP_POS;
			g_Bullet[nCntBullet].rot = rot;
			g_Bullet[nCntBullet].scl = { 1.0f, 1.0f, 1.0f };
			g_Bullet[nCntBullet].use = true;
			g_Bullet[nCntBullet].time = 0;
			// 影の設定
