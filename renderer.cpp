...
static ID3D11VertexShader*		g_VertexBlurShader = NULL;		//ブラーフィルタ用頂点シェーダー
...
static ID3D11PixelShader*		g_PixelBlurShader = NULL;		//ブラーフィルタ用ピクセルシェーダー
...
static ID3D11InputLayout*		g_VertexBlurLayout = NULL;		//ブラーフィルタ書き込み先
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	//ブラーフィルタ頂点シェーダー
	hr = D3DX11CompileFromFile("blur.hlsl", NULL, NULL, "VertexShaderPolygon", "vs_4_0", shFlag, 0, NULL, &pVSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "VS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), NULL, &g_VertexBlurShader);

	// 入力レイアウト生成
	D3D11_INPUT_ELEMENT_DESC layoutBlur[] =
	{
		{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "NORMAL",   0, DXGI_FORMAT_R32G32B32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "COLOR",    0, DXGI_FORMAT_R32G32B32A32_FLOAT,	0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	...
	UINT numBlurElements = ARRAYSIZE(layoutBlur);

	g_D3DDevice->CreateInputLayout(layoutBlur,
		numWaterElements,
		pVSBlob->GetBufferPointer(),
		pVSBlob->GetBufferSize(),
		&g_VertexBlurLayout);
	...
	// ブラーフィルタピクセルシェーダコンパイル・生成
	pPSBlob = NULL;
	hr = D3DX11CompileFromFile("blur.hlsl", NULL, NULL, "PixelShaderPolygon", "ps_4_0", shFlag, 0, NULL, &pPSBlob, &pErrorBlob, NULL);
	if (FAILED(hr))
	{
		MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "PS", MB_OK | MB_ICONERROR);
	}

	g_D3DDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, &g_PixelBlurShader);
...
void UninitRenderer(void)
{
	...
	if (g_VertexBlurLayout)		g_VertexBlurLayout->Release();
	if (g_VertexBlurShader)		g_VertexBlurShader->Release();
	if (g_PixelBlurShader)		g_PixelBlurShader->Release();
...
void SwapShader(int flag)
{
	...
	case MODE_SPECULAR:
		...
		break;
	case MODE_BLUR:
		g_ImmediateContext->VSSetShader(g_VertexBlurShader, NULL, 0);
		g_ImmediateContext->PSSetShader(g_PixelBlurShader, NULL, 0);
		g_ImmediateContext->IASetInputLayout(g_VertexBlurLayout);
